...
报警告:typing.Callable[[str], None] = lambda 警告信息: print(termcolor.colored("Warning：", "yellow") + 警告信息)
...
class 海康GigE工业相机:
	...
	@插值算法类型.setter
	def 插值算法类型(self, 插值算法类型: int | str | None = 2):
		...
		if (设定成败 != 0):
			报警告("设定插值算法模式失败，失败代码：0x{:x}".format(设定成败))
		else:
			if not(不回显信息):
	...
	def 曝光时间(self):
		...
		if (获取成败 != 0):
			报警告("获取曝光时间失败，失败代码：0x{:x}".format(获取成败))
			return 0
		else:
			# 转化为毫秒
	...
	def 图像捕获帧率(self):
		...
		if (获取成败 != 0):
			报警告("获取图像捕获帧率失败，失败代码：0x{:x}".format(获取成败))
			return 0
		else:
	...
	def 获取当前图像(self):
		...
		# 1000？
		图像获取成败 = self._摄像机对象.MV_CC_GetOneFrameTimeout(ctypes.byref(图像数据缓冲), 当前图片像素数, 帧数据, 1000)
		if (图像获取成败 != 0):
			match(初始状态):
				case 0:
					#不做任何事
...
def test():
	...
	眼A.增益 = 19
	眼B.增益 = 19
	cv2.imshow("A", cv2.resize(眼A.获取当前图像(),dsize=None,fx=0.3,fy=0.3))
	cv2.imshow("B", cv2.resize(眼B.获取当前图像(),dsize=None,fx=0.3,fy=0.3))
	眼A.关闭摄像头()
	眼B.关闭摄像头()
	cv2.waitKey()
